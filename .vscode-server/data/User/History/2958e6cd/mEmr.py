...
class UserCheck:
    def __init__(self, banned, message=None):# Here we set up the class to have the banned and
...
class CharCheck:
    def __init__(self, banned, message=None):# Here we set up the class to have the banned and message attributes.
    # banned must be passed through at declaration. 
    #       self.banned = banned        if not message:
            message = 'Unable to use special characters' # If no message chosen, then this default message is returned.
    ...
    def validate_username(self, username):
        ...
        for x in ban_char:
            for char in username :
                if char == x:
                    raise ValidationError('Cannot use special characters in username')
